...
async function run3DAnalysis() {
  ...
  try {
    ...
    if (isSTP) {
      ...
      if (stlData.wasCapped) {
        showToast('⚠️ 파일이 매우 큽니다. 4M 삼각형 한도로 자동 축약됩니다. STL 변환을 권장합니다.', 'warn');
      }
    } else {
    ...
    // 새 DFM 결과 카드들 업데이트
    updateThicknessInfoCard();
    updateValidationCard();
    updateMoldDesignCard(result.moldFeatures);
...
function updateAnalysisIssuesWithGate(gateResult) {
  ...
  updateAiReviewReport(App.stl.result, gateResult);
}
...
function animateScore(numId, ringId, gradeId, score) {
  ...
  // Color by score (Standardized CAD/CAE: Safe = Blue, Caution = Yellow, Danger = Red)
  let color, grade;
  if (score >= 85) { color = '#0055ff'; grade = 'SAFE'; }
  else if (score >= 70) { color = '#3b82f6'; grade = 'GOOD'; }
  else if (score >= 50) { color = '#ffd166'; grade = 'WARNING'; }
  else                  { color = '#ff4d6d'; grade = 'RISK'; }
...
function renderMoldFeatures(features) {
  ...
  container.style.display = 'flex';
  container.innerHTML = '<summary style="color: var(--cyan);">🔩 언더컷 피처 목록 (Features)</summary>';
...
function setupResizer(resizer, panel, direction) {
  ...
  function doDrag(e) {
    ...
    if (direction === 'left') {
      ...
      newWidth = Math.max(160, Math.min(newWidth, 